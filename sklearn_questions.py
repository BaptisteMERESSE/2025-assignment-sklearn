...
flake8 at the root of the repo.
...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        X = validate_data(X, ensure_2d=True, dtype=np.float64, reset=False)
        # compute Euclidean distances between X and training data
        dists = pairwise_distances(X, self.X_)
        # find nearest neighbor for each test sample
        idx_nearest = np.argmin(dists, axis=1)
        y_pred = self.y_[idx_nearest]
        return y_pred
